...
defineTypeNameAndDebug(coupledStabilised, 0);
addToRunTimeSelectionTable
(
    physicsModel, coupledStabilised, solid
);
addToRunTimeSelectionTable
(
    solidModel, coupledStabilised, dictionary
);


// * * * * * * * * * * * *  Private Member Functions * * * * * * * * * * * * //

void coupledStabilised::writeBanner(messageStream& os) const
{
    os  << nl
        << "/*---------------------------------------------------------------------------*\\\n"
        << "\\*---------------------------------------------------------------------------*/"
...
coupledStabilised::coupledStabilised
(
    Time& runTime,
    const word& region
)
...
for (int i = 0; i<3; i++){
    ...
    //     )
    //     {
    //         WarningIn("coupledStabilised::evolve()")
...
tmp<vectorField> coupledStabilised::tractionBoundarySnGrad
(
